...
class RestaurantDetailSerializer(serializers.Serializer):
    ...
    def get_promotion(self, obj):
        now = timezone.now()
        promotions = obj.promotions.filter(is_archived=False, ended_at__gte=now).order_by(
            '-created_at',
        )
        return PromotionSerializer(promotions, many=True).data if promotions.exists() else None

    def get_coupon(self, obj):
        now = timezone.now()
        coupons = obj.coupons.filter(is_archived=False, ended_at__gte=now).order_by('-started_at')
    ...
    def get_reviews(self, obj):
        ...
        return SimpleReviewSerializer(reviews, many=True).data if reviews.exists() else None
...
class FullRestaurantSerializer(serializers.ModelSerializer):
    ...
    def get_hasAvailableCoupon(self, obj):
        ...
        coupons = (
            ...
            .annotate(claimed_count=Count('claimed_by'))
        )
        return any(
            coupon.total is None or coupon.claimed_count < coupon.total for coupon in coupons
        )
